...
class SaleDocumentRepository extends EntityRepository
{
    public function findSaleDocuments()
    {
        ...
        $limitdate = new \DateTime('-6 months');

        //Query Builder
        $q = $this->createQueryBuilder("s")
            ->select('s.id,s.deliverydate,s.documentdate,s.documentnumber,s.customername,s.numberprefix, s.customerid, s.deliverystate ')
            ->where("s.numberprefix = :prefix") 
            ->andWhere('s.documentdate > :limitdate')
            ->andWhere("s.deliverystate = :etat")
            ->setParameter('prefix', 'CAT')
            ->setParameter('limitdate', $limitdate->format('d-m-y'))
            ->setParameter('etat', 0)
            ->orderBy('s.documentdate', 'ASC')
            ->getQuery();
